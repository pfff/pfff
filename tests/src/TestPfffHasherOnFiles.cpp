// Smoke test for PfffHasher on test files
#include "config.h"
#include "PfffHasher.h"
#include "PfffBlockReader.h"
#include "MTwister.h"

namespace TestPfffHasherOnFiles {
...
ofstream fo("TestPfffHasherOnFiles.outx");
...
TEST_FILEFIXTURE("TestPfffHasherOnFiles.out", TestPfffHasherOnFiles) {
	...
	for (int format_code_i = 0; format_code_i < NUM_FORMAT_CODE; format_code_i++) {
		...
		for (int data_i = 0; data_i < NUM_DATA; data_i++) {
			...
			LocalFileBlockReader* br = new LocalFileBlockReader(fn.str().c_str());
			ostringstream o;
			h->hash(o, br);
