/**
 * disk-read-experiment-2.cpp
 *
 * The program takes a list of files, and for each files reads m blocks of size <k> with skips of size 2MB, measuring time required for the read.
 *
 */
#include <iostream>
#include <vector>
...
const int BS[] = { 20*1024, 40*1024, 60*1024, 80*1024, 100*1024,
                    120*1024,140*1024,160*1024,180*1024,200*1024,
                    220*1024,240*1024,260*1024,280*1024,300*1024,
                    320*1024,340*1024,360*1024,380*1024,400*1024};
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < filenames.size(); i++) {
        ...
        timer.Start();
            for (int k = 0; k < bc; k++) {
                in.seekg(2*1024*1024*(k+1), ios::beg);
                if (method == 0) {
                    if (!in.read(buffer, 1)) {
