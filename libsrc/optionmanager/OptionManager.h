...
public:
	...
	CharPtrOption(char** target_variable, char* default_value):
		target_variable(target_variable), default_value(default_value) {}
...
struct OptionDef {
	...
	char* param_name;
	char* description; // For OPTION_GROUP this is just its name, for other options, 
	                   // it's the text that will go in the help message.
	
	OptionDef(OptionType type, char* long_option_name, char short_option_name, 
	          int* presence_counter, OptionReader* option_reader, char* param_name, char* description):
		type(type),
...
protected:
	 ...
	 */
	void add_unparameterized(char* long_option, char short_option, int* presence_counter, char* description);
	
	/**
	 * Used on OptionManager initialization, adds a "parameterized"-type OptionDef to the vector of available options.
	 */
	void add_parameterized(char* long_option, char short_option, int* presence_counter, OptionReader* option_reader, char* param_name, char* description);
